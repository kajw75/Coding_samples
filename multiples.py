...
try:
    ...
    print("Invalid or missing Input filename argument, please check that there are no typos!")
    exit(0)
...
try:
    ...
    print("Missing output filename argument, please check that there are no typos!")
    exit(0)
...
# Variables
    ...
    result2 = 0
    end_result = []
# Split the content lines using 'new line' as separator
    split_lines = c.split("\n")
# Remove trailing whitespace(s)
    c.rstrip()
# Display line contents on screen without string apostrophes
    print(split_lines[0])
# Handle each line's values separately
    for x in c:
# Split line values using 'space' as separator
        num = c.split(" ")
# Calculate the 1st values multiples and cast the string to integer
...
# Calculate the 2nd values multiples and cast the string to integer
...
# Reorder the end list to acending order
        end_result.sort()
# Remove list brackets & commas and cast the integer value as string using 'space' as separator
